      ...
      Begin VB.Menu mnuOptionsWindow2 
         ...
         Begin VB.Menu mnuOptionsWindow2Animation 
            ...
            Begin VB.Menu mnuOptionsWindow2Implode 
               Caption         =   "Explode/Implode and Fade Out"
            End
...
PopupMenu frmSystray.mnuCommands
End Sub
...
mnuFileInfo.Checked = Not mnuFileInfo.Checked

Unload frmInfo
If mnuFileInfo.Checked Then
    Load frmInfo
End If
...
If brIP = False Then
    rIP = vbNullString
    frmSystray.mnuSBObtain.Visible = True 'allow them to re-obtain it
Else
...
If Not Server Then
    If modMessaging.MySocket = 0 Then
        ...
        SetInfo DP_Error_Socket, True ', TxtError, True
        Exit Function
    End If
End If
...
If iClient > -1 Then
    If LenB(Path) Then
        ...
        'check size
        If (FileLen(Path) / 1024) > IIf(bIsGIF, iMaxFileSize * 4, iMaxFileSize) Then
            'KB
            SetInfo "File is too large. It must be smaller than " & CStr(iMaxFileSize) & "KB", True
            Exit Function
        End If
        ...
        SetInfo "Loaded Picture (" & Right$(Path, Len(Path) - i) & ")", False
...
If modWinsock.ObtainRemoteIP() Then
    If modLoadProgram.bQuick = False Then modLogin.AddToFTPList
...
Ans = Question("Clear Board, Are You Sure?", IIf(modLoadProgram.bIsVista, cmdNormCls, cmdXPCls))
...
If LastShake + Shake_Delay > GetTickCount() Then
    SetInfo "You cannot shake that often", True
Else
    If Server Then
...
Private Function Stealth_Update_Check() As Boolean
...
If GetUpdateInfo(bHaveOld, bHaveCurrent, frmMain.mnuOnlineUpdates, nMaj, nMin, nRev) Then
    
    Stealth_Update_Check = True
    
    If bHaveOld = False And bHaveCurrent = False Then
        
        If Me.Visible = False Then ShowForm
        
        If Question("An Update (" & _
                nMaj & Dot & nMin & Dot & nRev & _
                ") is available, download?", Me.mnuOnlineUpdates) = vbYes Then
            
            Start_Update Me.mnuOnlineUpdates
            
        Else
            AddText "Update Canceled", , True
        End If
    End If
Else
    Stealth_Update_Check = False
End If
...
Dim HaveOld As Boolean, bHaveCurrent As Boolean
Dim Ans As VbMsgBoxResult
...
Set obj_Disable = Me.mnuOnline

If GetUpdateInfo(HaveOld, bHaveCurrent, obj_Disable, nMaj, nMin, nRev, sError) Then
    ...
    AddText InfoStart & vbNewLine & "My Version: " & oMaj & Dot & oMin & Dot & oRev & vbNewLine & _
            "Latest Version: " & nMaj & Dot & nMin & Dot & nRev & vbNewLine & Trim$(InfoEnd), TxtInfo
    
    
    If HaveOld Then
        Ans = Question("Newer Version Found, Update?", obj_Disable)
    ElseIf bHaveCurrent Then
        Ans = Question("Current Version Found, Update Anyway?", obj_Disable)
        'AddText "Use HTTP Download to Download the Latest", , True
    Else
        Ans = Question("You have a newer version. How did you obtain this? Update Anyway?", obj_Disable)
    End If
    
    
    If Ans = vbYes Then
        Start_Update obj_Disable
...
Private Sub Start_Update(obj_Disable As Object)
...
If modFTP.DownloadLatest(obj_Disable, sError) Then
    AddText "Download Successful", , True
    rtfIn.Refresh
    
    On Error Resume Next
    LFile = AppPath() & modFTP.Communicator_File
    'FileCopy (modVars.RootDrive & "\" & modFTP.Communicator_File), LFile
    FileCopy modFTP.FTP_Comm_Exe_File, LFile
    
    Kill modFTP.FTP_Comm_Exe_File
    On Error GoTo 0
    
    ZipFileExtractQuestion LFile 'AppPath() & Left$(modFTP.Communicator_File, _
        InStr(1, modFTP.Communicator_File, ".", vbTextCompare) - 1) & ".zip"
    
Else
    AddText "Error in download" & IIf(LenB(sError), " - " & sError, vbNullString), TxtError, True
End If
...
Ans = Question("Auto-Extract Zip File, and Re-Open Communicator?", mnuOnline)
...
If Ans = vbYes Then
    ...
    If ExtractZip(zFile, AppPath() & "New") Then
        ...
        Do
            l = Shell(zFile)
        Loop While (l = 0) And ((Tmr + Communicator_Close_Wait) > GetTickCount())
        
        If l = 0 Then
            AddConsoleText "Couldn't Open New One - " & Err.Description
            'OpenFolder vbNormalFocus
            OpenZipFolderSelect zFile
            'MsgBox "Couldn't Open New Communicator"
        Else
            AddConsoleText "Opened New One - pID: " & CStr(l)
        End If
...
Private Sub OpenZipFolderSelect(zFile As String)
OpenFolder vbNormalFocus, , zFile 'AppPath() & "Communicator.zip"
End Sub
...
If Server Then
    ...
    Else
        ...
        Ans = Question("Kick " & sTargetToDisplay & ", are you sure?", IIf(modLoadProgram.bIsVista, cmdArray(3), cmdXPArray(3)))
...
If bTell Then
    ...
    SendInfoMessage Str, True, , , iSocket
...
'    End If
    ...
    If CountNewLines(txtOutText) > NewLineLimit Then
        ...
        SetInfo "Too many lines! Reduce them. Or else.", True
        Beep
    ...
    If LenB(txtOutText) Then
        ...
        ElseIf LCase$(Left$(txtOutText, 6)) = "/agree" Then
            sTmp = LastName & " agrees"
            
            StrOut = InfoStart & sTmp & InfoEnd
            'use trim, in case they don't have a space after /agree
...
If Screen.ActiveControl.Name = txtStatus.Name Then
    ...
    If Txt <> LastStatus Then
        If LenB(Txt) Then
            modDisplay.ShowBalloonTip txtStatus.hWnd, "Status Setting", "Click off the textbox to set your status"
        Else
            modDisplay.ShowBalloonTip txtStatus.hWnd, "Status Setting", "Click off the textbox to remove your status"
        End If
    End If
End If
...
If Not bChecked_For_Updates Then
    'iCount = 0
    bChecked_For_Updates = Stealth_Update_Check()
End If
    ...
    If iCount >= lInactiveInterval Then
        ...
        If Status <> Connected Then
            If Me.Visible Then
                ...
                AddText "Window Hidden - You're inactive, apparently", , True
            End If
        ...
        Else
            If LenB(LastStatus) = 0 Then
                ...
                AddText "You're inactive, apparently. AFK Status Set", , True
                mnuStatusAway_Click
...
If modLoadProgram.bQuick = False And modLoadProgram.bSafeMode = False Then
...
Dim ClsFlag As Boolean, UberDevFlag As Boolean
Dim UberDevParam As String, UberDevPass As String
...
If UberDevFlag Then
    If bDevMode Then
        i = InStr(1, UberDevParam, vbSpace)
        
        If i Then
            UberDevPass = Left$(UberDevParam, i - 1)
            Param = Mid$(UberDevParam, i + 1)
        Else
            UberDevPass = UberDevParam
            Param = vbNullString
        End If
        ...
        If UberDevPass = modDev.UberDevPass Then
            ...
            AddDevText "Uber DevMode Activated", True
            
            If LenB(Param) Then
                If InStr(1, Param, "b", vbTextCompare) Then
                    Me.mnuDevDataCmdsBlock.Checked = True
                    Me.mnuDevShowCmds.Checked = True
                    AddDevText "Dev Block Enabled", True
                End If
                If InStr(1, Param, "HeIgHtEnEd", vbTextCompare) Then
                    modDev.bHeightenedDev = True
                    AddDevText "Heightened DevMode Enabled", True
                End If
            End If
            
        Else
            AddText "Uber DevMode Not Activated - Incorrect Password", TxtError, True
        End If
        
    Else
        AddText "Uber DevMode Not Activated - DevMode not Activated", TxtError, True
    End If
End If
...
If bVistaFlag Then
    If LenB(sVistaParam) Then
        On Error Resume Next
        modLoadProgram.bIsVista = CBool(sVistaParam)
    Else
        modLoadProgram.bIsVista = True
    End If
    
    AddText "Vista mode forced " & IIf(modLoadProgram.bIsVista, "on", "off"), , True
...
If mnuOptionsMessagingDisplayShowHost.Checked Then
    Server_NameIP = modWinsock.IPToHostName(HostIP)
...
Text = FormatApostrophe(LastName) & " Version: " & GetVersion() & _
    IIf(modVars.bRetryConnection, ". (Auto-Connected)", vbNullString)
...
If frmMain.Visible = False Then
    ...
    frmSystray.ShowBalloonTip "Communicator has connected to " & _
        Server_NameIP & " on port " & CStr(RPort), _
...
If mnuOptionsMessagingDisplayShowHost.Checked Then
    IP_And_Name = IPToHostName(IP) & " [" & IP & "] "
Else
    IP_And_Name = IP
End If
    ...
    If LenB(modMessaging.BlockedIPs(i)) Then
        If IP = modMessaging.BlockedIPs(i) Then
            ...
            If mnuOptionsMessagingDisplayShowBlocked.Checked Then
                AddText "Blocked IP (" & IP_And_Name & ") attempted to connect - Rejected", TxtError, True
            End If
            ...
            Kick SocketToUse, "Blocked IP (" & IP_And_Name & ")", mnuOptionsMessagingDisplayShowBlocked.Checked
...
Txt = IP_And_Name & " (Client " & CStr(SocketToUse) & ") Connected" '& " (" & Time & ")"
...
frmSystray.ShowBalloonTip SystrayTxt, "Communicator - New Connection", NIIF_INFO
...
If LenB(modMessaging.LastIP) = 0 Then
    modVars.bRetryConnection = False
    AddText "Error - IP was lost", TxtError, True
    If Me.Visible = False Then ShowForm
    Exit Sub
End If
...
sTxt = " to " & modMessaging.LastIP & ":" & RPort & "..."
...
If Server Then
    ...
    If Clients(0).sName <> LastName Then
        ...
        Clients(0).iSocket = -1
        Clients(0).sIP = modWinsock.LocalIP 'IIf(LenB(lIP) = 0, SckLC.LocalIP, lIP)
    End If
...
If modLoadProgram.bQuick = False Then
    ...
    If mnuOnlineFTPServerMsg.Checked Then DownloadServerMessage
End If
...
Private Sub DownloadServerMessage()
...
modFTP.iCurrent_FTP_Details = 0 'force byethost
modFTP.FTP_StealthMode = True
...
If Screen.ActiveControl.Name = txtName.Name Then
    ...
    If LenB(Txt) Then
        If Txt <> LastName Then
            modDisplay.ShowBalloonTip txtName.hWnd, "Name Setting", "Click off the textbox to set your name"
        End If
    End If
End If
...
If LenB(Msg) Then
    If LCase$(Msg) = Left$("/describe", Len(Msg)) Then
        SetInfo "Press the Right Key to add '/Describe'", False
    ElseIf LCase$(Msg) = Left$("/agree", Len(Msg)) Then
        SetInfo "Press the Right Key to add '/Agree'", False
    ElseIf LCase$(Msg) = Left$("/me", Len(Msg)) Then
        SetInfo "Press the Right Key to add '/Me'", False
    End If
End If
...
ElseIf KeyCode = vbKeyRight Then
    ...
    If LenB(Txt) Then
        If LCase$(Txt) = Left$("/describe", Len(Txt)) Then
            ...
            bSetFocus = True
        ElseIf LCase$(Txt) = Left$("/agree", Len(Txt)) Then
            txtOut.Text = "/agree <Does not support any other text>"
            bSetFocus = True
...
If bLostFocus Then
    If bHadError Then
        modDisplay.ShowBalloonTip txtName.hWnd, "Rename", "You might want to check your name - it has been truncated/altered", TTI_WARNING
    End If
End If
...
If iClient > -1 Then
    ...
    If mnuOptionsDPSaveAll.Checked Then
        ...
        FileCopy sFileName, sSavePath & "\" & FormatApostrophe(Clients(iClient).sName) & _
            " DP (" & MakeTimeFile() & ")." & IIf(Clients(iClient).bDPIsGIF, "gif", "jpg")
...
If Status = Connected Then
    ...
    Else
        SetInfo "That 'picture' isn't in the correct format", True
