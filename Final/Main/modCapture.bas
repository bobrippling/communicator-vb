...
Private Const SWP_NOSENDCHANGING As Long = &H400&   ' /* Don't send WM_WINDOWPOSCHANGING */
Private Const HWND_BOTTOM As Long = 1&

Public Declare Function DestroyWindow Lib "user32" (ByVal hWnd As Long) As Long 'C BOOL
...
Private Const DelayMS = 1000 / FPS

Public Type ptDevice
    Index As Integer
    sName As String
    sVersion As String
End Type
...
Public Type VFWPOINT 'strange name to avoid collision with other POINT UDTs
        X As Long
        Y As Long
End Type
...
Public Declare Function capGetDriverDescription Lib "avicap32.dll" Alias "capGetDriverDescriptionA" _
...
End Function
Function capPreview(ByVal hCapWnd As Long, ByVal bP As Boolean) As Boolean
capPreview = SendMessageAsLong(hCapWnd, WM_CAP_SET_PREVIEW, -(bP), 0&) 'convert the VB Boolean to a C BOOL with the - sign
End Function
...
Public Sub ResizeCaptureWindow(ByVal hCapWnd As Long)
'Dim retVal As Boolean
'Dim capStat As CAPSTATUS
Const wFlag = SWP_NOMOVE Or SWP_NOZORDER 'Or SWP_NOSENDCHANGING

'Resize it to fit picCap
SetWindowPos hCapWnd, HWND_BOTTOM, 0&, 0&, frmCapture.picCap.ScaleWidth, frmCapture.picCap.ScaleHeight, wFlag
...
'    'SetWindowPos frmCapture.hWnd, 0&, 0&, 0&, capStat.uiImageWidth, capStat.uiImageHeight, wFlag
'
'    ''Resize the capture window to format size
'    'SetWindowPos hCapWnd, 0&, 0&, 0&, capStat.uiImageWidth, capStat.uiImageHeight, wFlag
'
'End If

End Sub

Public Function VBEnumCapDrivers(ByRef Devices() As ptDevice) As Long
'/*
...
For i = 0 To MaxVidDrivers - 1
    If modCapture.capGetDriverDescription(CLng(i), Device, Cap_String_Max, Version, Cap_String_Max) Then
        
        ReDim Preserve Devices(i)
        
        Devices(i).Index = i
        Devices(i).sName = Left$(Device, InStr(Device, vbNullChar) - 1)
        Devices(i).sVersion = Left$(Version, InStr(Version, vbNullChar) - 1)
...
If capDriverConnect(hCapWnd, nDriverIndex) Then
    
    '// Get the capabilities of the capture driver
    If capDriverGetCaps(hCapWnd, Caps) Then
        With frmCapture
            .cmdSource.Enabled = CBool(Caps.fHasDlgVideoSource)
            .cmdFormat.Enabled = CBool(Caps.fHasDlgVideoFormat)
            .cmdDisplay.Enabled = CBool(Caps.fHasDlgVideoDisplay)
            '.cmdOverlay.Enabled = CBool(Caps.fHasOverlay)
        End With
        
        '//Set preview scale (fit into window)
        Call capPreviewScale(hCapWnd, True)
        
        '// Set the preview rate in milliseconds
        Call capPreviewRate(hCapWnd, DelayMS)
...
'        'frmCapture.Caption = LPSTRtoVBString(lpStatusString)
'
'        'or, just pass the LPCSTR to a WINAPI function
'        'Call SetWindowTextAsLong(frmCapture.hWnd, lpStatusString)
'
'End Select
'
''Debug.Print "Driver returned code " & StatusCode & " to StatusProc"
'
'StatusProc = 1 'or "-(True)" => converts Boolean to C BOOL
'
'End Function
